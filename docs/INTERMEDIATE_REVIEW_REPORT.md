...
본 보고서는 팀별 프로젝트 과제(수요 코딩회)에서 구현된 "독립 카운터 & 생명주기 타이머(MVP)"와 이를 지탱하는 Custom React Core 엔진을 대상으로 합니다. Week 2/3 과정에서 구현했던 순수 Virtual DOM 엔진을 어떻게 재사용하여 동적인 React 컴포넌트로 확장시켰는지 그 연결고리와 완성도를 비판적으로 평가합니다.
